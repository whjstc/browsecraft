...
如果你想用 Firefox 系反检测后端，而不是 Chromium，那么 Camoufox 更合适。

BrowseCraft 目前支持两种接法：

1. 直接让 BrowseCraft 启动 Camoufox：`browsecraft start --type camoufox`
2. 连接一个已存在的 Camoufox websocket 端点：`browsecraft connect <ws-endpoint> --type camoufox`

前置准备：

- 安装 Camoufox 包并下载浏览器二进制
- 或者通过 `--camoufox-path` 显式指定二进制路径
...
如果你想自己把 Camoufox 作为远程服务跑起来，官方文档给出的方式是：

```bash
python -m camoufox server
```

它会打印一个 websocket 端点，例如：

```text
ws://localhost:1234/hello
```

然后就可以让 BrowseCraft 接过去：
...
- Camoufox 远程 server 在官方文档里仍然是 experimental
- 一个 server 对应一个浏览器实例，不会自动在多个 session 间轮换指纹
- BrowseCraft 当前没有像 Chrome 一样，为 Camoufox 提供一等公民的 `--profile` 抽象
- 如果你今天就要在 Camoufox 上保持长期登录态，更稳妥的做法是维护一个长期运行的 Camoufox 实例，或者把状态生命周期管理放在 Camoufox 侧
