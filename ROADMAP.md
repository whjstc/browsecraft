...
| **发布循环脚本** | `scripts/release-loop.sh`（test + preflight + skill 导出） | 15 分钟 | ✅ 已完成 |
| **README 更新** | 命令数量、已完成能力、待发布能力拆分清楚 | 30 分钟 | ✅ 已完成 |
| **Skill 包骨架** | 建立 `packages/skill`，沉淀统一 prompt / metadata / 示例 | 1 小时 | ✅ 已完成 |
| **Skill 提交资产导出** | `scripts/export-skill-assets.sh`（`packages/skill/dist/*`） | 15 分钟 | ✅ 已完成 |
...
| **Skill 兼容矩阵** | 记录三平台字段差异与适配策略 | 1 小时 | ✅ 已完成（`packages/skill/platform-matrix.md`） |
...
| **tab/frame 管理** | `tab new/close/switch`、`frame list/switch/clear` | 30 分钟 | ✅ 已完成 |

### P3 - 高级特性（2-3 个月）

| 功能 | 说明 | 工作量 | 状态 |
|------|------|--------|------|
| **工作流引擎** | YAML 工作流定义 + AI 步骤支持 | 2 小时 | 🚧 部分完成（YAML run/validate/dry-run 已完成，AI 步骤待做） |
...
| 单元测试覆盖不足（已建立基础用例） | 代码质量 | P2 |
| 缺少集成测试 | 稳定性 | P2 |
| Skill 平台实际上架依赖人工登录与审核 | 上架效率与可维护性 | P1 |
